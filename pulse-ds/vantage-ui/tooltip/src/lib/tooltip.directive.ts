...
import { VcTooltip } from './tooltip.component';
...
export class VcTooltipDirective {
  @Input() VcTooltip: string | null = '';  
  ...
  onMouseMove($event: MouseEvent): void {
    if (
      ...
      this.componentRef.instance.VcTooltip = this.VcTooltip;
    }
  }
  ...
  private initializeTooltip() {
    if (this.componentRef === null) {
      ...
      const componentFactory =
        this.componentFactoryResolver.resolveComponentFactory(VcTooltip);
    ...
    if (this.componentRef !== null) {
      ...
      switch (this.position) {
        case TooltipPosition.BELOW: {
          ...
          this.componentRef.instance.top = Math.round(bottom) + window.scrollY;
          break;
        }
        case TooltipPosition.ABOVE: {
          ...
          this.componentRef.instance.top = Math.round(top) + window.scrollY;
          break;
        }
        case TooltipPosition.RIGHT: {
          this.componentRef.instance.left = Math.round(right);
          this.componentRef.instance.top =
            Math.round(top + (bottom - top) / 2) + window.scrollY;
          break;
        }
